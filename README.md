...
* память Python-объектов через `tracemalloc`;
* пиковое потребление памяти во время запроса;
...
### 2. Python memory через `tracemalloc`
...
## Как определяется подозрение на утечку
...
* если удержанная память устойчиво растёт на протяжении окна наблюдения, библиотека помечает эндпоинт как `suspected_leak=true`.
...
* сериализацию ответа;
* работу `response_model`;
* память, выделенную уже после возврата значения из функции.
...
* `config.py` — dataclass-конфигурация и модели метрик.
* `decorator.py` — декоратор `@memory_watch(...)`.
* `detector.py` — эвристика определения подозрения на утечку.
* `route.py` — основной перехватчик FastAPI-маршрутов.
* `sinks.py` — логирование метрик в файл в человекочитаемом виде.
* `__init__.py` — публичные экспорты библиотеки.
...
`tracemalloc` входит в стандартную библиотеку Python и отдельно не устанавливается.

## Быстрый старт

### Шаг 1. Подключить библиотеку в `main.py`
...
### Шаг 2. Создать роутер с `route_class=MemoryWatchRoute`
...
Для анализа памяти рекомендуется использовать именно один worker.

## Почему нужен один worker
...
И желательно без `--reload`, так как авто-перезагрузка тоже создаёт шум по памяти.

## Важно: оба условия обязательны

Чтобы метрики снимались, нужно выполнить **оба** условия:

1. у роутера должен быть `route_class=MemoryWatchRoute`;
2. на endpoint должен быть навешан `@memory_watch(...)`.
...
#### `Пиковое потребление во время запроса`
...
* высокий `Пиковое потребление во время запроса`;
* небольшой `Удержанная память`;
* `Признаки утечки: не обнаружены`;
...
## Когда есть серьёзное подозрение на leak
...
## Пример тестового роутера для локальной проверки
...
## Сценарии локальной проверки

### 1. Проверка, что библиотека подключена

```bash
curl http://127.0.0.1:8000/debug-memory/ok
```

Ожидаемый результат:

* запрос отвечает `200`;
* в файл лога записывается блок метрик.

### 2. Проверка большого потребления памяти без leak

```bash
curl "http://127.0.0.1:8000/debug-memory/heavy?size_mb=20"
```
...
### 3. Проверка подозрения на leak

Несколько раз подряд:

```bash
curl "http://127.0.0.1:8000/debug-memory/leak?size_mb=5"
```

Ожидаемый результат:

* по мере накопления `LEAK_BUCKET` растёт retained memory;
* после заполнения окна наблюдения endpoint может получить `Признаки утечки: обнаружены`.

## Где смотреть лог-файл
...
```powershell
Get-Content .\logs\memory_watch.log
```

Просмотр в реальном времени:

```powershell
Get-Content .\logs\memory_watch.log -Wait
```

### Просмотр на Linux

```bash
cat logs/memory_watch.log
```

В реальном времени:

```bash
tail -f logs/memory_watch.log
```
...
## Декоратор `memory_watch`

Пример:

```python
@memory_watch(name="city.get_cities")
```
...
Человекочитаемое имя endpoint, которое попадает в лог.
...
### 1. Начинайте с локального тестового роутера
...
* либо отключить `tracemalloc`, если нужен минимальный overhead;
* либо включать библиотеку только на ограниченном наборе endpoint;
...
1. библиотека не может со 100% точностью доказать утечку памяти по одному вызову;
2. рост RSS не всегда означает leak;
3. Python allocator не всегда возвращает память ОС сразу;
4. `tracemalloc` видит только Python-аллокации, а не всю native memory;
5. при высокой конкурентности показатели становятся шумнее;
6. при нескольких worker-процессах интерпретация усложняется.
...
### Не появляются метрики

Проверьте:

* вызван ли `configure_memory_watch(...)`;
* указан ли `route_class=MemoryWatchRoute` у роутера;
* навешан ли `@memory_watch(...)` на endpoint;
* корректен ли путь `log_file_path`;
* есть ли права на запись в директорию логов.
...
Используйте абсолютные импорты вместо запуска файла с относительными импортами.
...
а запускать лучше так:

```bash
uvicorn main:app --host 127.0.0.1 --port 8000 --workers 1
```

### Лог пустой или не создаётся
...
* отдельный alert-файл только для `suspected_leak=true`;
* фильтрация top allocations только по файлам проекта;
...
1. сколько памяти съедает конкретный endpoint;
2. какой пик памяти возникает во время обработки запроса;
3. есть ли признаки накопления памяти между одинаковыми вызовами.
